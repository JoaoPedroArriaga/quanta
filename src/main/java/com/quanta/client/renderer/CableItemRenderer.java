package com.quanta.client.renderer;
...
public class CableItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public static CableItemRenderer getInstance() {
        if (instance == null) {
            instance = new CableItemRenderer();
        }
        return instance;
    }
    
    private CableType getTypeFromItem(ItemStack stack) {
        if (stack.getItem() instanceof QuantaCableItem) {
            return CableType.ENERGY;
        } else if (stack.getItem() instanceof QuantumItemPipeItem) {
            return CableType.ITEM;
        } else if (stack.getItem() instanceof QuantumLiquidPipeItem) {
            return CableType.LIQUID;
        } else if (stack.getItem() instanceof QuantumPressurizedPipeItem) {
            return CableType.GAS;
        }
        return CableType.ENERGY;
    }
    
    private ResourceLocation getTextureForType(CableType type) {
        return ResourceLocation.fromNamespaceAndPath(Quanta.MOD_ID, "block/" + type.textureName);
    }
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack,
        ...
        CableType type = getTypeFromItem(stack);
        ResourceLocation texture = getTextureForType(type);
        ...
        // Ajusta posição e rotação baseado no contexto
        ...
        // Pega a textura correta
        var sprite = Minecraft.getInstance().getModelManager().getAtlas(net.minecraft.world.inventory.InventoryMenu.BLOCK_ATLAS)
            .getSprite(texture);
        
        // Pega os quads do modelo base
        ...
        // Renderiza cada quad com a textura substituída
